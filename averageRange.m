function averageRange( varargin )
...
if nargin > 1
    ...
    xlsxPath = fullfile(tablesDir,['averagerange_cs_',timestamp,'.xlsx']);
end
...
for iObj = 1:nObj
   ...
   % 
   hoursIdx =  hour(objArray(iObj).Time) >= 5 & hour(objArray(iObj).Time) <= 10;
...
T = table(id, group, session, protocol, meanRangeCS, GeomMeanRangeCS);
