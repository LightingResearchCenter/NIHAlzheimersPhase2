...
if nargin > 1
    ...
    xlsxPath = fullfile(tablesDir,['mean_log10CircadianLight_',timestamp,'.xlsx']);
end
...
for iObj = 1:nObj
    ...
    meanLogCla(iObj,:) = (objArray(iObj).hourly(log10(objArray(iObj).CircadianLight), 'mean', lux10idx))';
...
T = table(id, group, session, protocol, meanLogCla);
